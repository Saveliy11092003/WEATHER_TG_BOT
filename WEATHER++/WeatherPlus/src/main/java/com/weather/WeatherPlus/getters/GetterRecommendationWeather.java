...
public class GetterRecommendationWeather {
    ...
    Translator translatorRu = new TranslatorRu();
    Translator translatorEn = new TranslatorEn();

    public GetterRecommendationWeather(UserRepository userRepository, Long chatId, StoreUnits storeUnits, boolean isRus) {
    ...
    public String getRecommendation() throws IOException {
        log.info("Went into the method getRecommendation");
        ...
        if (temperature >= 30) {
            ...
            answer = EmojiParser.parseToUnicode("\uD83D\uDD25 ") +
                    getCorrectStr("The sun is scorching outside, so you should stay at home if possible." +
            ...
            if (precipitation.equals("rain")) {
                answer += EmojiParser.parseToUnicode("\n\n\uD83C\uDF27 ") +
                        getCorrectStr("It's also raining outside. You need to take an umbrella or a thin raincoat with you.");
            }
            log.info("getRecommendation returned " + answer);
            return answer;
        }

        if (temperature >= 20) {
            answer = EmojiParser.parseToUnicode(":sunny: ") +
                    getCorrectStr("It’s warm outside, so you should wear comfortable clothes made from light natural materials. " +
        ...
        if (temperature >= 10) {
            answer = EmojiParser.parseToUnicode(":relaxed: ") + getCorrectStr("It's cool outside, " +
            ...
            if (precipitation.equals("rain")) {
                answer += EmojiParser.parseToUnicode("\n\n\uD83C\uDF27 ") + getCorrectStr("It's also raining outside, " +
        ...
        if (temperature >= 0) {
            answer = EmojiParser.parseToUnicode("\uD83C\uDF24 ") + getCorrectStr("To go outside today, you should wear a warm jacket, " +
                    "and don’t forget about a hat, scarf and gloves!");
            if (precipitation.equals("rain") || precipitation.equals("snow")) {
                answer += EmojiParser.parseToUnicode("\n\n:cloud: ") + getCorrectStr("It's ") + getCorrectStr(precipitation) +
                        getCorrectStr("ing outside now, so you need to take an umbrella with you and wear warmer shoes.");
            }
        ...
        if (temperature >= -10) {
            answer = EmojiParser.parseToUnicode("\uD83C\uDF2C ") + getCorrectStr("It's getting cold outside, " +
                    "so you should put on a warmer jacket or coat, take out a hat and gloves.");
            if (precipitation.equals("rain") || precipitation.equals("snow")) {
                answer += EmojiParser.parseToUnicode("\n\n:cloud: ") + getCorrectStr("It's " + precipitation) +
                        getCorrectStr("ing now, so remember that in such conditions it is important " +
        ...
        if (temperature >= -20) {
            answer = EmojiParser.parseToUnicode(":snowman: ") + getCorrectStr("It feels cold outside, so you need to dress warmly! " +
                    "Today you should wear a sweater and warm pants, and a down jacket on top.");
            if (precipitation.equals("snow")) {
                answer += EmojiParser.parseToUnicode("\n\n\uD83C\uDF28 ") + getCorrectStr("It's also snowing outside, " +
        ...
        if (temperature >= -30) {
            answer = EmojiParser.parseToUnicode("\uD83E\uDDCA ") + getCorrectStr("The temperature is quite low now, so you need to wear a warm insulated coat, " +
                    "warm trousers and shoes. It is important not to forget about gloves, a scarf and a hat!");
            if (precipitation.equals("snow")) {
                answer += EmojiParser.parseToUnicode("\n\n\uD83C\uDF28 ") + getCorrectStr("It's also snowing, " +
        ...
        answer = EmojiParser.parseToUnicode("\uD83E\uDD76 ") + getCorrectStr("Temperatures outside are very low at the moment, " +
        ...
        if (precipitation.equals("snow")) {
            answer += EmojiParser.parseToUnicode("\n\n\uD83C\uDF28 ") + getCorrectStr("It's snowing outside, so wear waterproof shoes!");
        }

        log.info("Exited the method getRecommendation");
        return answer;
    }
